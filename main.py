...
drop_variables = ['ID','Month','Age','SSN','Occupation','Num_Bank_Accounts','Num_Credit_Card','Num_of_Loan',
                  'Type_of_Loan','Payment_of_Min_Amount','Payment_Behaviour'] # Variables that won´t be used
customer_variables = ['Name','Annual_Income','Monthly_Inhand_Salary','Interest_Rate','Credit_Mix',
                      'Credit_History_Age'] # Variables that will be filled with the same info of the customer
cero_variables = ['Num_of_Delayed_Payment','Num_Credit_Inquiries','Amount_invested_monthly',
                  'Monthly_Balance'] # Variables that will be filled with 0
numeric_variables = ['Annual_Income','Num_of_Delayed_Payment','Changed_Credit_Limit','Outstanding_Debt',
            'Amount_invested_monthly','Monthly_Balance'] # Variables that need to be on numerical
outliers_variables = ['Annual_Income','Interest_Rate','Num_of_Delayed_Payment','Num_Credit_Inquiries',
                      'Total_EMI_per_month','Amount_invested_monthly','Monthly_Balance'] # Variables with outliers
model_variables = ['Annual_Income','Monthly_Inhand_Salary','Interest_Rate','Delay_from_due_date',
                  'Num_of_Delayed_Payment','Changed_Credit_Limit','Num_Credit_Inquiries','Credit_Mix',
                  'Outstanding_Debt','Credit_Utilization_Ratio','Credit_History_Age'] # Variables used on the model
## Explore the data
train_info = f.analyse_data(train) # Analyse the train data
test_info = f.analyse_data(test) # Analyse the test data
## Clean the data and make all numeric values for the classifiers
df_train =  f.data_cleaning(train,drop_variables,customer_variables,cero_variables,numeric_variables,
                            outliers_variables)
df_test =  f.data_cleaning(test,drop_variables,customer_variables,cero_variables,numeric_variables,
                           outliers_variables).fillna(0)
replace_cm = {"Credit_Mix": {"Good": 0, "Standard": 1, "_": 2, "Bad": 3}}
replace_cs = {"Credit_Score": {"Good": 0, "Standard": 1, "Poor": 2}}
df_train.replace(replace_cm, inplace=True)
df_train.replace(replace_cs, inplace=True)
df_test.replace(replace_cm, inplace=True)
## Get the x and y of the train, validation and test datasets
features = df_train[model_variables]
labels = df_train['Credit_Score']
x_train, x_validation, y_train, y_validation = train_test_split(features, labels, test_size=0.30, random_state=42) 
x_test = df_test[model_variables]
ct = ColumnTransformer([('standardize', Normalizer(), model_variables)], remainder='passthrough')
...
# ## Stacking (Support Vector Machine, Random Forest, K Neighbors and Gradient) Classifier
# estimators = [("Support Vector Machine", SVC()),
